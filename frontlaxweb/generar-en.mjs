...
function construirFaq(html, idioma) {
  ...
  // en un <details><summary><h3>...</h3><svg chevron/></summary> para el acordeón, con el
  // chevron de por medio. Las dos formas valen.
  const pares = [
    ...seccion[1].matchAll(/<h3[^>]*>([\s\S]*?)<\/h3>(?:[\s\S]*?<\/summary>)?\s*<p[^>]*>([\s\S]*?)<\/p>/g),
  ];
